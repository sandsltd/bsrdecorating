...
const FeaturedProjects = ({ category, title, subtitle }: FeaturedProjectsProps) => {
  ...
  const defaultSubtitle = category === 'domestic'
    ? 'See our latest home transformation projects across Devon'
    : 'Professional commercial decorating projects for businesses across Devon';
  ...
  }, [selectedProject, featuredProjects]);
  ...
  return (
    <section className="py-16 px-4 sm:px-6 lg:px-8 bg-bsr-gray">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12">
          <h2 className="text-4xl font-bold text-bsr-white mb-4">
            {(title || defaultTitle).split(' ').map((word, index, array) => 
      ...
      {/* Image Carousel Modal - Same as GalleryPreview */}
      {selectedProject && (() => {
        const project = featuredProjects.find(p => p.id === selectedProject);
